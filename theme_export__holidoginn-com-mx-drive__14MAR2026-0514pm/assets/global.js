...
document.addEventListener("DOMContentLoaded", function () {
  // ✅ Reemplazo del label de términos y condiciones
  const interval = setInterval(() => {
    const labels = document.querySelectorAll("label");

    labels.forEach((label) => {
      if (label.textContent.includes("Terminos y Condiciones Holidog Inn")) {
        const checkbox = label.querySelector("input[type='checkbox']");
        label.textContent = "Acepto los";

        const link = document.createElement("a");
        link.href = "/pages/terminos-y-condiciones";
        link.target = "_blank";
        link.style = "color: #007bff; text-decoration: underline; margin-left: 5px;";
...
// ✅ Función para actualizar total estimado
function updateEstimatedTotal() {
  console.log("🟡 Ejecutando updateEstimatedTotal, noches:", nights);

  const selectedDays = document.querySelectorAll('.neo-cell-selected-bookeasy');
  const nights = selectedDays.length;
  ...
  // ⬇️ Ahora lo insertamos justo debajo del selector de cantidad
  const priceContainer = document.querySelector('[id^="Quantity-"]')?.parentElement;
  if (!priceContainer) return;

  let estText = document.querySelector('#estimated-total');
  ...
  if (!estText) {
    estText = document.createElement('div');
    estText.id = 'estimated-total';
    estText.style.marginTop = '10px';
    estText.style.color = '#444';
    estText.style.padding = '10px 0';
    estText.style.fontWeight = '500';

    const addToCartButton = priceContainer.querySelector('button[name="add"]');
    if (addToCartButton) {
      priceContainer.insertBefore(estText, addToCartButton);
    } else {
      priceContainer.appendChild(estText);
      console.log("✅ Total estimado insertado:", estText);

    }

  }

  const newText = `<strong>Total estimado:</strong> $${estimatedTotal.toLocaleString()} MXN<br><small>(se pagará al llevar a tu mascota)</small>`;
  if (estText.innerHTML !== newText) {
    estText.innerHTML = newText;
  }
  if (!document.body.contains(estText)) {
  updateEstimatedTotal(); // vuelve a llamar si fue eliminado
}
}
...
document.addEventListener('DOMContentLoaded', () => {
  const checkInterval = setInterval(() => {
    const calendar = document.querySelector('.neo-calendar'); // o el contenedor principal de BookX

    const quantityBlock = document.querySelector('[id^="Quantity-"]');
    if (calendar && quantityBlock) {
      updateEstimatedTotal();
